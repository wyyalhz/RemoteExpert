...
void AVReceiver::processReceivedData(const QByteArray& data) {
    ...
    try {
        ...
        switch (type) {
        case MsgType1::VIDEO_FRAME:
            if (m_videoEnabled) {
                processVideoFrame(jsonData, binaryData);
            }
            break;
        case MsgType1::AUDIO_FRAME:
            if (m_audioEnabled) {
                processAudioFrame(jsonData, binaryData);
            }
            break;
        case MsgType1::SCREEN_FRAME:
            if (m_videoEnabled) {
                processScreenFrame(jsonData, binaryData);
            }
            break;
        case MsgType1::TEXT_MESSAGE:
        case MsgType1::SYSTEM_MESSAGE:
...
void AVReceiver::processTextMessage(const QJsonObject& jsonData, MsgType1 type) {
    TextMessage message;
    ...
    if (ProtocolPackager::parseTextMessage(jsonData, message)) {
        ...
        // 如果是系统消息，设置正确的类型
        if (type == MsgType1::SYSTEM_MESSAGE) {
