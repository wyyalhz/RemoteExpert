...
QByteArray ProtocolPackager::packVideoFrame(uint32_t roomId,
    ...
    return packMessage(MsgType1::VIDEO_FRAME, jsonObj, frameData);
}
...
QByteArray ProtocolPackager::packAudioFrame(uint32_t roomId,
    ...
    return packMessage(MsgType1::AUDIO_FRAME, jsonObj, audioData);
}


QByteArray ProtocolPackager::packMessage(MsgType1 type,
...
bool ProtocolPackager::unpackMessage(const QByteArray& data,
    ...
    type = static_cast<MsgType1>(ntohs(netType));
...
QByteArray ProtocolPackager::packControlCommand(MsgType1 commandType, uint32_t roomId, const QJsonObject& extraData) {
    QJsonObject jsonObj;
    jsonObj["roomId"] = static_cast<int>(roomId);
...
QByteArray ProtocolPackager::packScreenFrame(uint32_t roomId,
    ...
    return packMessage(MsgType1::SCREEN_FRAME, jsonObj, frameData);
}
...
QByteArray ProtocolPackager::packTextMessage(uint32_t roomId,
    ...
    return packMessage(MsgType1::TEXT_MESSAGE, jsonObj);
}
QByteArray ProtocolPackager::packSystemMessage(uint32_t roomId,
    ...
    return packMessage(MsgType1::SYSTEM_MESSAGE, jsonObj);
}
